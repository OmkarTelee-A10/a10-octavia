...
class A10OctaviaNeutronDriver(AllowedAddressPairsDriver):
    ...
    def create_port(self, network_id, subnet_id, fixed_ip=None):
        new_port = None
        try:
            port = {'port': {'name': 'octavia-port-' + network_id,
                             'network_id': network_id,
            ...
            message = _("Error creating port in network: {0}".format(network_id))
            LOG.exception(message)
    ...
    def delete_port(self, port_id):
        try:
            ...
            message = _("Error deleting port: {0}".format(port_id))
            LOG.exception(message)
